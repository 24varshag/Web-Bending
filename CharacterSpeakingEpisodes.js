...
d3.csv('avatar_transcripts_with_context.csv').then(function(data) {
    ...
    d3.selectAll('.dot').on('mouseover', function(event) {
        ...
        const dialogue = getCharacterDialogue(character, episode);
...
function showTooltip(event, dialogue) {
    ...
    tooltip.style.top = `${event.clientY + 15}px`;  // 15px below the mouse pointer
    tooltip.style.left = `${event.clientX + 15}px`;  // 15px to the right of the mouse pointer
}
...
function getCharacterDialogue(character, episode) {
    // Use the global 'data' loaded by d3.csv() to find the dialogue for the character in the episode
    const dialogueData = window.data.find(d => d.Character === character && d['Episode Number'] == episode);
