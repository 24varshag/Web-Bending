...
let activeCharacters = new Set(["Aang", "Katara", "Sokka", "Toph", "Zuko", "Iroh"]);
...
function setupFilters() {
  ...
  const allCheckboxes = document.querySelectorAll('#filter-options input[type="checkbox"]:not(#select-all)');
...
function moveCharactersToEpisode(targetEpisode) {
  ...
  characterData.forEach(entry => {
    if (parseInt(entry.episode) === targetEpisode && activeCharacters.has(entry.character)) {
