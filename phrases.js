...
function updatePhrasesGraph() {
  ...
  if (!svg.empty()) {
    svg.transition().duration(fadeDuration).style("opacity", 0).remove();
  }
  ...
  setTimeout(() => {
    ...
    const fullKey =
      selectedEpisode === "All"
        ? selectedSeason
        : `${selectedSeason} - Episode ${selectedEpisode}`;

    let phrases =
      (phraseData[selectedCharacter] &&
        phraseData[selectedCharacter][fullKey]) ||
      [];
    ...
    node
      ...
      .each(function (d) {
        ...
        if (words.length <= 3) {
          g.append("tspan").attr("x", 0).attr("y", 0).text(d.text);
        } else {
          const firstLine = words
            .slice(0, Math.ceil(words.length / 2))
            .join(" ");
          const secondLine = words.slice(Math.ceil(words.length / 2)).join(" ");
          g.append("tspan").attr("x", 0).attr("y", "-0.6em").text(firstLine);
          g.append("tspan").attr("x", 0).attr("y", "0.9em").text(secondLine);
        }
      })
    ...
    // Fade in nicely
    newSvg.transition().duration(fadeDuration).style("opacity", 1);
